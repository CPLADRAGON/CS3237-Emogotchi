...
void Task_OLED(void *pvParameters){
  // 静态变量，用于跟踪最后绘制的状态
  // 初始化为一个无效值 (-1)，以强制在第一次运行时重绘
  static LedState oledLastState = (LedState)-1; 

  while(1) {
    // 复制 volatile 变量到本地副本，以便安全比较
    LedState currentStateCopy = g_currentState;

    // 检查当前状态是否已更改
    if (currentStateCopy != oledLastState) {
      
      // 状态已更改，需要重绘。
      // 获取 I2C 锁，因为 OLED 和 MPU 共享 I2C 总线
      if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        
        display.clearDisplay(); // 清除屏幕

        const unsigned char* iconToDraw; // 指向要绘制的图标
