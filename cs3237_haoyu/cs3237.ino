...
// #define SCREEN_WIDTH 64  // <--------------------------------------------------------------------------------------------------------------------------------  移除 OLED
// #define SCREEN_HEIGHT 48 // <--------------------------------------------------------------------------------------------------------------------------------  移除 OLED
...
void onMqttConnect(esp_mqtt_client_handle_t client)
{
    ...
    {
        log_i("MQTT 已连接 (来自 onMqttConnect 回调), 正在订阅...");
        mqttClient.subscribe(mqtt_topic, [](const std::string &payload)
                             { log_i("%s: %s", mqtt_topic, payload.c_str()); });
